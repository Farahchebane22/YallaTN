...
public class ActivityReceiptPdfService {
    ...
    private String normalizeBaseUrl(String baseUrl) {
        String base = baseUrl == null ? "http://localhost:4200" : baseUrl.split(",")[0].trim();
    ...
    private String resolveLocalLogoAsDataUri() {
        try (InputStream in = ActivityReceiptPdfService.class.getClassLoader()
                .getResourceAsStream("static/assets/logo/yallatn-logo.png")) {
            if (in != null) {
                byte[] bytes = in.readAllBytes();
                if (bytes.length > 0) {
                    return "data:image/png;base64," + Base64.getEncoder().encodeToString(bytes);
